...
def embed(request, *elements, **kw):
    ...
    if as_user is not None and not request._indexing_view:
        ...
        if cached is None:
            ...
            # cannot cache individual item audit because _referenced_uuids will
            # change depending on the original item triggering the audits
            if '@@audit-self' not in path:
...
def _embed(request, path, as_user='EMBED'):
    ...
    # do this because we want the referenced_uuids generated by @@embedded
    # for the @@audit view as part of the indexing process.
    # Will default to @@object referenced_uuids if this
