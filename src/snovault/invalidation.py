...
def add_to_indexing_queue(success, request, uuid, edit_or_add):
    """
    Add item to queue for indexing. This function should be called from
    addAfterCommitHook. See item_edit and collection_add in .crud_view.py.
    ...
    if success:  # only queue if the transaction is successful
        try:
            ...
            if indexer_queue:
                ...
                indexer_queue.add_uuids(request.registry, [uuid], strict=(edit_or_add == 'add'))
                if indexer_queue_mirror:
                    indexer_queue_mirror.add_uuids(request.registry, [uuid], strict=(edit_or_add == 'add'))
            else:
                ...
                if es:
                    raise Exception("Indexer queue not configured! Attempted to queue %s for method %s.' % (uuid, edit_or_add)")
        except Exception as e:
            error_msg = repr(e)
    else:
        error_msg = 'Transaction not successful! %s not queued for method %s.' % (uuid, edit_or_add)
    if error_msg:
        log.error('___Error queueing %s for indexing. Error: %s' % (uuid, error_msg))
